...
class InsufficientCopiesError(BookstoreError):
    def __init__(self, book_title, requested, available):
        ...
        super().__init__(
            f"cannot sell {requested} of {book_title}: only {available} in stock, short by {self.shortage}"
        )
...
class Bookstore:
    ...
    def total_value(self):
       ...
       #     sum_b = sum(b["price"] * b["quantity"])
       #     return round(sum_b, 2)
        return round(sum(b["price"] * b["quantity"] for b in self.books.values()), 2)
